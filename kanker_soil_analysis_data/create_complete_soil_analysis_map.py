#!/usr/bin/env python3
"""
Create comprehensive NPK+Boron+Iron+Zinc+Soil pH map showing all seven nutrients
"""
...
def create_comprehensive_all_nutrients_with_ph_map():
    """Create interactive map with Nitrogen, Phosphorus, Potassium, Boron, Iron, Zinc, and Soil pH zones"""
    ...
    # Add Soil pH zones
    for zone_name, zone_info in soil_ph_zones.items():
        folium.Rectangle(
            bounds=[[zone_info["lat_range"][0], zone_info["lon_range"][0]], 
                   [zone_info["lat_range"][1], zone_info["lon_range"][1]]],
            color=zone_info["color"],
    ...
    for village in data['village_wise_data']['villages']:
        if 'coordinates' in village and len(village['coordinates']) == 2:
            ...
            <div style="width: 340px;">
                ...
                <h5 style="color: #8A2BE2; margin: 5px 0;">🧪 Soil pH</h5>
                <p><b>Level:</b> {village.get('soil_ph_level', 'N/A')}</p>
                <p><b>Range:</b> {village.get('estimated_soil_ph', 'N/A')}</p>
                <p><b>Zone:</b> {soil_ph_zone}</p>
            ...
            folium.Marker(
                ...
                tooltip=f"{village['village_name']} - N:{nitrogen_level}, P:{village.get('phosphorus_level', 'N/A')}, K:{village.get('potassium_level', 'N/A')}, B:{village.get('boron_level', 'N/A')}, Fe:{village.get('iron_level', 'N/A')}, Zn:{village.get('zinc_level', 'N/A')}, pH:{village.get('soil_ph_level', 'N/A')}"
            ).add_to(m)
    
    # Add comprehensive legend
    legend_html = f'''
    <div style="position: fixed; 
                bottom: 50px; left: 50px; width: 580px; height: 550px; 
                background-color: white; border:2px solid grey; z-index:9999; 
                font-size:9px; padding: 10px; border-radius: 5px; box-shadow: 0 2px 5px rgba(0,0,0,0.2);">
        <h4 style="margin-top:0; color: #333; text-align: center;">Kanker District Complete Soil Analysis</h4>
        
        <div style="margin: 2px 0;">
            <h5 style="color: #2E8B57; margin: 1px 0;">🌱 Nitrogen Zones:</h5>
            <div style="margin: 1px 0; font-size: 7px;">
        ...
        <div style="margin: 2px 0;">
            <h5 style="color: #8A2BE2; margin: 1px 0;">🧪 Soil pH Zones:</h5>
            <div style="margin: 1px 0; font-size: 7px;">
                <span style="color: darkgreen;">■</span> Green (Normal): {village_stats["soil_ph"]["green"]} villages
            </div>
            <div style="margin: 1px 0; font-size: 7px;">
                <span style="color: darkorange;">■</span> Orange (Slightly Acidic): {village_stats["soil_ph"]["orange"]} villages
            </div>
            <div style="margin: 1px 0; font-size: 7px;">
                <span style="color: dimgray;">■</span> Grey (Moderately Acidic): {village_stats["soil_ph"]["grey"]} villages
            </div>
            <div style="margin: 1px 0; font-size: 7px;">
                <span style="color: gray;">■</span> Low: {village_stats["soil_ph"]["low"]} villages
            </div>
        </div>
        ...
        <div style="margin: 2px 0;">
            ...
            <div style="margin: 1px 0; font-size: 7px;">
                ...
                <span style="color: blue; margin-left: 3px;">●</span> Low-Medium N
                <span style="color: orange; margin-left: 3px;">●</span> Medium N
            </div>
            <div style="margin: 1px 0; font-size: 7px;">
                <span style="color: red;">●</span> High N
                <span style="color: darkred; margin-left: 3px;">●</span> Very High N
            </div>
        </div>
        
        <div style="margin: 8px 0 0 0; padding: 6px; background-color: #f0f0f0; border-radius: 3px;">
            <p style="margin: 0; font-size: 7px; color: #666; text-align: center;">
                <b>Total Villages:</b> {len(data['village_wise_data']['villages'])} | 
                <b>Complete NPK+Boron+Iron+Zinc+Soil pH Data</b>
            </p>
        </div>
    </div>
    '''
    ...
    # Save map
    map_filename = "kanker_complete_soil_analysis_map.html"
    m.save(map_filename)
    
    print(f"✅ Complete Soil Analysis Map Created: {map_filename}")
    ...
    print(f"   - Soil pH Zones: {village_stats['soil_ph']['green']} Green + {village_stats['soil_ph']['orange']} Orange + {village_stats['soil_ph']['grey']} Grey + {village_stats['soil_ph']['low']} Low")
    print(f"   - Total Villages: {len(data['village_wise_data']['villages'])}")
    
    return map_filename

if __name__ == "__main__":
    create_comprehensive_all_nutrients_with_ph_map()

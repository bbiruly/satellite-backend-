...
logger = logging.getLogger("landsat_indices")
logger.setLevel(logging.WARNING)

# Landsat-specific thresholds
LANDSAT_THRESHOLDS = {
...
def compute_landsat_indices(red: np.ndarray, nir: np.ndarray, swir1: np.ndarray, swir2: np.ndarray, blue: np.ndarray) -> Dict[str, float]:
    """Compute vegetation indices for Landsat data"""
    indices = {}
...
def estimate_npk_from_landsat_indices(indices: Dict[str, Any], 
                                     region: Region = Region.GLOBAL,
                                     crop_type: CropType = CropType.GENERIC) -> Dict[str, float]:
    """Estimate NPK values from Landsat vegetation indices using dynamic coefficients"""
    ...
    # Landsat-specific NPK estimation with dynamic coefficients
    # These are calibrated for Landsat's 30m resolution and different spectral bands
    
    # Nitrogen estimation (Landsat calibration with dynamic coefficients)
    nitrogen = coeffs.nitrogen_base + (ndvi * coeffs.nitrogen_ndvi * 0.6) + (savi * coeffs.nitrogen_savi * 0.8)
    nitrogen = max(coeffs.nitrogen_min * 0.3, min(coeffs.nitrogen_max * 0.4, nitrogen))  # Adjusted for Landsat resolution
    
    # Phosphorus estimation (Landsat calibration with dynamic coefficients)
    phosphorus = coeffs.phosphorus_base + (ndvi * coeffs.phosphorus_ndvi * 0.7) + (ndmi * coeffs.phosphorus_ndwi * 0.6)
    phosphorus = max(coeffs.phosphorus_min * 0.2, min(coeffs.phosphorus_max * 0.4, phosphorus))  # Adjusted for Landsat resolution
    
    # Potassium estimation (Landsat calibration with dynamic coefficients)
    potassium = coeffs.potassium_base + (ndvi * coeffs.potassium_ndvi * 0.7) + (savi * coeffs.potassium_savi * 0.6)
    potassium = max(coeffs.potassium_min * 0.25, min(coeffs.potassium_max * 0.5, potassium))  # Adjusted for Landsat resolution
    
    # SOC estimation (Landsat calibration with dynamic coefficients)
    soc = coeffs.soc_base + (ndvi * coeffs.soc_ndvi * 0.8) + (ndmi * coeffs.soc_ndmi * 0.7)
    soc = max(coeffs.soc_min * 0.3, min(coeffs.soc_max * 0.75, soc))  # Adjusted for Landsat resolution
...
def compute_indices_and_npk_for_bbox_landsat(bbox: Dict[str, float], 
                                            start_date: Optional[datetime] = None, 
                                            end_date: Optional[datetime] = None,
                                            crop_type: str = "GENERIC") -> Dict[str, Any]:
    """Compute Landsat vegetation indices and NPK for bounding box with dynamic coefficients"""
    try:
        ...
        logger.info(f"🌍 Landsat Region detected: {region.value}")
        logger.info(f"🌱 Landsat Crop type: {crop_enum.value}")
        ...
        if not items:
            return {"success": False, "error": "No Landsat data found for specified date range"}
        ...
        # Compute indices
        indices = compute_landsat_indices(red_array, nir_array, swir1_array, swir2_array, blue_array)
        
        # Estimate NPK
        npk = estimate_npk_from_landsat_indices(indices, region, crop_enum)
        ...
        logger.error(f"Landsat processing error: {str(e)}")
